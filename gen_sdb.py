...
function toggleUnit() {
  ...
  if (typeof updateDepthLabels === 'function') updateDepthLabels();
  if (typeof updateThermalDepthLabels === 'function') updateThermalDepthLabels();
}
function toggleTempUnit() {
  ...
  if (typeof updateThermalLabels === 'function') updateThermalLabels();
}
...
(function(){
  ...
  function _placeGps(ll,acc){
    if(!_mk){
      _mk=L.marker(ll,{icon:L.divIcon({className:'',html:'<div class="gps-pulse-wrap"><div class="gps-pulse-dot"></div></div>',iconSize:[18,18],iconAnchor:[9,9]}),zIndexOffset:1000}).addTo(map);
  ...
  if(h1){h1.title='Recentrer le lac';h1.addEventListener('click',function(){
  ...
  function _addSimRow(){
    ...
    row.innerHTML='<label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:11px;color:#94a3b8"><input type="checkbox" id="_gpsSimChk" style="accent-color:#f59e0b"> 📍 Simuler GPS (clic sur carte)</label>';
    dp.appendChild(row);
    document.getElementById('_gpsSimChk').addEventListener('change',function(){
      _simActive=this.checked;map.getContainer().style.cursor=_simActive?'crosshair':'';
    ...
    map.on('click',function(e){
      ...
      _simMk=L.marker(ll,{icon:L.divIcon({className:'',html:'<div style="width:16px;height:16px;background:#f59e0b;border-radius:50%;border:3px dashed #fff;box-shadow:0 0 8px rgba(245,158,11,.6)"></div>',iconSize:[16,16],iconAnchor:[8,8]}),zIndexOffset:999,title:'Position simulée'}).addTo(map);
      _placeGps(ll,8);var b=document.getElementById('btnGps');if(b)b.className='gps-ctrl-btn gps-on';
    });
  }
...
if('serviceWorker'in navigator){navigator.serviceWorker.register('./sw.js',{scope:'./'}).catch(function(){});}
    ...
    # Inject PWA head tags if not already present
    if 'rel="manifest"' not in html:
        m = _re.search(r'(<meta\s[^>]*charset[^>]*>)', html, _re.I)
        if m:
            html = html[:m.end()] + '\n' + PWA_HEAD + html[m.end():]

    # Inject new GPS/offline/sim block before </body>
    if '</body>' in html:
        html = html.replace('</body>', GPS_BLOCK + '\n</body>', 1)
    else:
        html += '\n' + GPS_BLOCK
...
def build_pdf_html(data: dict, lac_config: dict, template_path: str) -> str:
    ...
    html = re.sub(r'<h1>[^<]*</h1>', f'<h1>\U0001F3A3 Lac {lac_name}</h1>', html, count=1)
    ...
    # ── PWA: manifest link, GPS control, service worker registration ──────────
