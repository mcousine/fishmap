...
def _inject_pwa(html: str) -> str:
    """Add PWA manifest link, GPS Leaflet control, and service worker registration."""
    import re as _re
...
.gps-pulse-dot{position:absolute;top:2px;left:2px;width:14px;height:14px;background:#06b6d4;
  border-radius:50%;border:2px solid #fff;animation:gpsRing 1.6s ease-out infinite}
@keyframes gpsRing{
...
if('serviceWorker'in navigator){navigator.serviceWorker.register('./sw.js',{scope:'./'}).catch(function(){});}
    ...
    # Inject PWA head tags after charset meta
    m = _re.search(r'(<meta\s[^>]*charset[^>]*>)', html, _re.I)
    if m:
        html = html[:m.end()] + '\n' + PWA_HEAD + html[m.end():]

    # Inject GPS + SW before </body>
    if '</body>' in html:
